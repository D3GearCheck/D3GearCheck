...
client.itemToHtml = function(item) {
    if (item !== undefined) {
        var html = "<a href=\"http://us.battle.net/d3/en/item/" + this.strToSlug(item.name) + "\">" +
...
client.addItemToDoll = function(slot, item) {
    switch (slot) {
        ...
        case "shield":
            this.insertItem("slot_secondary", item);
            break;
...
client.loadSkillScreen = function() {
    ...
    this.loadout.updateForSkills(this.className, this.skills.active);
